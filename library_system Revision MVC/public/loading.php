...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validasi data admin di database
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $error = "Nama, Username, atau Password salah. Silakan coba lagi."; // Pesan error jika login gagal
    }
}
?>
...
<body class="bg-gray-100 flex items-center justify-center h-screen">
    <div class="text-center p-6 bg-white rounded-lg shadow-lg w-full max-w-sm">
        ...
        <!-- Admin Login Form -->
        <form action="loading.php" method="POST" class="space-y-4">
            <div class="flex flex-col">
                <label for="nama" class="text-left text-gray-700 font-medium">Nama</label>
                <input type="text" name="nama" id="nama" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Nama Admin">
            </div>
            <div class="flex flex-col">
                <label for="username" class="text-left text-gray-700 font-medium">Username</label>
                <input type="text" name="username" id="username" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Username Admin">
            </div>
            <div class="flex flex-col">
                <label for="password" class="text-left text-gray-700 font-medium">Password</label>
                <input type="password" name="password" id="password" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Password Admin">
            </div>
            <button type="submit" class="w-auto bg-blue-500 text-white py-2 px-6 rounded-lg hover:bg-blue-600 mt-4">
                Login as Admin
            </button>
        </form>
